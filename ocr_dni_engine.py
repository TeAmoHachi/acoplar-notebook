...
def parsear_dni(texto_ocr: str) -> dict:
    ...
    # ═══════════════════════════════════════════════════════
    # 6️⃣ ESTADO CIVIL (buscar S, C, D, V)
    # ═══════════════════════════════════════════════════════
    
    for i, linea in enumerate(lineas):
        # 🔥 NUEVO: Tolerar errores OCR (Civil, Civit, Ciwil, etc.)
        if re.search(r'Estado\s*Ci[vwt][a-z]*', linea, re.IGNORECASE):
            # Revisar las siguientes 5 líneas
            for j in range(i + 1, min(i + 6, len(lineas))):
                if lineas[j] in ['S', 'C', 'D', 'V']:
                    ...
                    datos["estado_civil"] = mapa[lineas[j]]
