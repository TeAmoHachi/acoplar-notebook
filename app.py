...
def formulario_base(valores: dict | None = None, disabled: bool = False):
    ...
    if mostrar_padre:
        ...
        # ==========================================
        # 🆕 OPCIÓN 3: ESCANEAR DNI CON OCR
        # ==========================================
        st.markdown("---")
        st.caption("**O escanea el DNI con IA:**")
        
        col_ocr1, col_ocr2 = st.columns([4, 1])
        with col_ocr1:
            uploaded_dni_padre = st.file_uploader(
                "📸 Sube imagen del DNI del Padre (JPG/PNG)",
                type=["jpg", "jpeg", "png"],
                key="ocr_padre",
                disabled=disable_lookup
            )
        with col_ocr2:
            ...
            st.write("")
            en_transicion = st.session_state.get("_modo_transitorio", False)
            procesar_ocr_padre = st.button(
                "🤖 Escanear", 
                key="btn_ocr_padre", 
                disabled=(disable_lookup or en_transicion)
            )

        # Procesar OCR del PADRE
        if procesar_ocr_padre and uploaded_dni_padre:
            import tempfile
            from ocr_dni_engine import extraer_datos_dni
            
            # Guardar imagen temporal
            with tempfile.NamedTemporaryFile(delete=False, suffix=".jpg") as tmp:
                tmp.write(uploaded_dni_padre.getbuffer())
                tmp_path = tmp.name
            
            with st.spinner("🤖 Extrayendo datos del DNI con IA..."):
                datos_ocr = extraer_datos_dni(tmp_path)
            
            if "error" in datos_ocr:
                st.error(f"❌ {datos_ocr['error']}")
                st.info("""
                **Posibles causas:**
                - La imagen está borrosa o mal iluminada
                - El DNI está incompleto o cortado
                - Es un documento extranjero no soportado
                
                **Solución:** Toma una foto clara del DNI completo con buena luz.
                """)
            else:
                st.success("✅ DNI escaneado exitosamente con IA")
                
                # Rellenar campos automáticamente
                st.session_state["padre_nombre"] = datos_ocr.get("nombre_completo", "")
                st.session_state["padre_doc_num"] = datos_ocr.get("dni", "")
                st.session_state["padre_dni"] = datos_ocr.get("dni", "")
                
                # 🔥 FIX: Si detectó estado civil, guardarlo
                if datos_ocr.get("estado_civil"):
                    st.session_state["padre_estado_civil"] = datos_ocr.get("estado_civil")
                ...
                with col_info1:
                    st.info(f"""
                    📋 **Datos extraídos:**
                    - **DNI:** {datos_ocr.get('dni', 'N/A')}
                    - **Nombre:** {datos_ocr.get('nombre_completo', 'N/A')}
                    """)
                with col_info2:
                    st.info(f"""
                    📊 **Información adicional:**
                    - **Fecha Nac:** {datos_ocr.get('fecha_nacimiento', 'N/A')}
                    - **Edad:** {datos_ocr.get('edad', 'N/A')} años
                    - **Sexo:** {datos_ocr.get('sexo_completo', 'N/A')}
                    - **Estado Civil:** {datos_ocr.get('estado_civil', 'N/A')}
                    """)
                
                st.success("✅ Datos cargados automáticamente. Revisa los campos abajo.")
                st.rerun()
    ...
    if mostrar_madre:
        ...
        # ==========================================
        # 🆕 OPCIÓN 3: ESCANEAR DNI CON OCR (MADRE)
        # ==========================================
        st.markdown("---")
        st.caption("**O escanea el DNI con IA:**")
        ...
        col_ocr1, col_ocr2 = st.columns([4, 1])
        with col_ocr1:
            uploaded_dni_madre = st.file_uploader(
                "📸 Sube imagen del DNI de la Madre (JPG/PNG)",
                type=["jpg", "jpeg", "png"],
                key="ocr_madre",
                disabled=disable_lookup
            )
        with col_ocr2:
            ...
            st.write("")
            en_transicion = st.session_state.get("_modo_transitorio", False)
            procesar_ocr_madre = st.button(
                "🤖 Escanear", 
                key="btn_ocr_madre", 
                disabled=(disable_lookup or en_transicion)
            )

        # Procesar OCR de la MADRE
        if procesar_ocr_madre and uploaded_dni_madre:
            import tempfile
            from ocr_dni_engine import extraer_datos_dni
    
            # Guardar imagen temporal
            with tempfile.NamedTemporaryFile(delete=False, suffix=".jpg") as tmp:
                tmp.write(uploaded_dni_madre.getbuffer())
                tmp_path = tmp.name
            ...
            if "error" in datos_ocr:
                ...
                **Solución:** Toma una foto clara del DNI completo con buena luz.
                ...
                # 🔥 FIX: Rellenar campos automáticamente
                st.session_state["madre_nombre"] = datos_ocr.get("nombre_completo", "")
                st.session_state["madre_doc_num"] = datos_ocr.get("dni", "")
                st.session_state["madre_dni"] = datos_ocr.get("dni", "")
        
                # 🔥 FIX: Si detectó estado civil, guardarlo
                if datos_ocr.get("estado_civil"):
                    st.session_state["madre_estado_civil"] = datos_ocr.get("estado_civil")
    ...
    with col_m1:
        ...
        # ==========================================
        # 🆕 OPCIÓN 3: ESCANEAR DNI CON OCR (MENOR)
        # ==========================================
        st.markdown("---")
        st.caption("**O escanea el DNI con IA:**")
        ...
        col_ocr1, col_ocr2 = st.columns([4, 1])
        with col_ocr1:
            uploaded_dni_menor = st.file_uploader(
                "📸 Sube imagen del DNI del Menor (JPG/PNG)",
                type=["jpg", "jpeg", "png"],
                key="ocr_menor",
                disabled=disable_lookup
            )
        with col_ocr2:
            ...
            st.write("")
            en_transicion = st.session_state.get("_modo_transitorio", False)
            procesar_ocr_menor = st.button(
                "🤖 Escanear", 
                key="btn_ocr_menor", 
                disabled=(disable_lookup or en_transicion)
            )

        # Procesar OCR del MENOR
        if procesar_ocr_menor and uploaded_dni_menor:
            import tempfile
            from ocr_dni_engine import extraer_datos_dni
            ...
            # Guardar imagen temporal
            with tempfile.NamedTemporaryFile(delete=False, suffix=".jpg") as tmp:
                tmp.write(uploaded_dni_menor.getbuffer())
                tmp_path = tmp.name
            ...
            if "error" in datos_ocr:
                ...
                **Solución:** Toma una foto clara del DNI completo con buena luz.
                ...
                # Rellenar campos automáticamente
                st.session_state["menor_nombre"] = datos_ocr.get("nombre_completo", "")
                st.session_state["menor_doc_num"] = datos_ocr.get("dni", "")
                st.session_state["menor_dni"] = datos_ocr.get("dni", "")
                
                # Convertir fecha ISO a date para el widget
                if datos_ocr.get("fecha_nacimiento_iso"):
                    try:
                        fnac_date = datetime.strptime(
                            datos_ocr["fecha_nacimiento_iso"], 
                            "%Y-%m-%d"
                        ).date()
                        st.session_state["menor_fnac"] = fnac_date
                    except:
                        pass
                
                # Detectar sexo automáticamente
                if datos_ocr.get("sexo"):
                    st.session_state["sexo_menor"] = datos_ocr["sexo"]
